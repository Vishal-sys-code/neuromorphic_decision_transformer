# src/run_pendulum.py
"""
Run a full comparison between SNN-DT and DSF-DT on Pendulum (continuous action).
This file is defensive and mirrors fixes we've applied to other run_*.py scripts.
"""
...
# Experiment config for pendulum
...
# Try to reuse dsf collector if available
try:
    from src.train_dsf_dt import collect_trajectories as dsf_collect_trajectories
except Exception:
    dsf_collect_trajectories = None

# Try to import real models; fall back to lightweight toys if not available
try:
    from src.models.snn_dt_patch import SNNDecisionTransformer as RealSNNDecisionTransformer
except Exception:
    RealSNNDecisionTransformer = None
...
# ---------------------------
# Simple fallback toy models (continuous-friendly)
# ---------------------------
class ToyDecisionSpikeFormer(nn.Module):
    ...
    def forward(self, states, actions_in=None, returns=None, returns_target=None, timesteps=None):
        ...
        h = self.mlp(h)
        action_pred = self.head(h)  # (B, L, act_dim)
        return None, action_pred, None


class ToySNNDecisionTransformer(nn.Module):
...
def collect_shared_dataset(env_name: str) -> Tuple[List[dict], int]:
    """
    Uses dsf_collect_trajectories(env_name, offline_steps, max_length) if available;
    otherwise uses a fallback collector.
    """
    ...
    if dsf_collect_trajectories is not None:
        ...
        while steps < offline_steps:
            ...
            # gym API: 5-tuple (obs, reward, terminated, truncated, info) or 4-tuple (obs, reward, done, info)
...
def train_model(model_class, trajectories, act_dim, env_name, is_continuous, model_name,
    ...
    # Prepare a local dt config (use the dt_config from your imported config module)
    ...
    # ensure position/context fields are present (DecisionTransformer expects these)
    if "n_positions" not in dt_conf_local:
        dt_conf_local["n_positions"] = dt_conf_local.get("max_length", max_length)
    # many DT implementations expect n_ctx too
    dt_conf_local.setdefault("n_ctx", dt_conf_local["n_positions"])
    
    # choose constructor (real model if available, else toy)
    if model_class == "SNN":
        ModelClass = RealSNNDecisionTransformer if RealSNNDecisionTransformer is not None else ToySNNDecisionTransformer
    else:
        ModelClass = RealDecisionSpikeFormer if RealDecisionSpikeFormer is not None else ToyDecisionSpikeFormer
    
    # Try to instantiate with keyword args (preferred). If the real DecisionTransformer uses positional args,
    # fallback to a common positional ordering (hidden_size, n_layer, n_head, n_inner, state_dim, act_dim, max_length).
    try:
        model = ModelClass(**dt_conf_local).to(device)
    except TypeError as e:
        # Fallback: try a positional-style constructor (best-effort)
        try:
            model = ModelClass(
                ...
                state_dim,
                int(act_dim),
                int(dt_conf_local.get("max_length", max_length))
            ).to(device)
        except Exception:
            # last resort: raise the original error to surface what's wrong
            raise e
...
def evaluate_model(model: nn.Module, env_name: str, n_eval: int = 20, device: str = DEVICE, seed: int = None):
    ...
    for _ in range(n_eval):
        ...
        while not done:
            ...
            # for continuous action, pick last timestep values
            if is_continuous:
...
def main():
    ...
    if (not args.recollect) and os.path.exists(data_path):
        ...
        # pass args.offline_steps and args.max_length into collector (compat with dsf_collect_trajectories)
        # dsf_collect_trajectories signature: (env_name, offline_steps, max_length)
        if dsf_collect_trajectories is not None:
            trajectories, act_dim = dsf_collect_trajectories(env_name, args.offline_steps, args.max_length)
        else:
    ...
    # Train SNN-DT
    print("\n=== Training SNNDecisionTransformer ===")
    snn_model, snn_hist = train_model("SNN", trajectories, act_dim, env_name, is_continuous,
                                      model_name="SNN-DT", device=args.device,
                                      epochs=args.epochs, batch_size=args.batch_size, lr=args.lr,
                                      max_length=args.max_length, gamma=gamma)

    # Train DSF-DT
    print("\n=== Training DecisionSpikeFormer ===")
    dsf_model, dsf_hist = train_model("DSF", trajectories, act_dim, env_name, is_continuous,
    ...
    # Evaluate
    print("\n=== Evaluating models ===")
    res_snn = evaluate_model(snn_model, env_name, n_eval=10, device=args.device, seed=args.seed)
    res_dsf = evaluate_model(dsf_model, env_name, n_eval=10, device=args.device, seed=args.seed)
