...
class Rotator:
    ...
    def reverse_half(q):
        ...
        """
        u = q[..., :q.shape[-1] // 2]  # 认为是各个二维子空间的第一维的向量集合
        v = q[..., q.shape[-1] // 2:]  # 认为是各个二维子空间的第二维的向量集合
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False, attn_index=-1):
        ...
        local_window_size = n_ctx # non-local
        print("local_window_size", local_window_size)
        # causal mask
        bias = torch.tril(torch.ones((n_ctx, n_ctx), dtype=torch.uint8))
        # # local mask
        bias = torch.triu(bias, -local_window_size+1)
        # register
        self.register_buffer(
            "bias", bias.view(1, 1, n_ctx, n_ctx)
        )
        ...
        # new add
        self.rotator = Rotator(n_state//config.n_head, torch.arange(n_ctx))


    # @staticmethod
    # def create_local_mask(seq_len, local_window_size):
    #     mask = torch.ones(seq_len, seq_len, dtype=torch.bool)
    #     # mask = torch.tril(mask, local_window_size-1) no need for causal mask
    ...
    def forward(
        ...
        # query = self.rotator.rotate(query)
        # key = self.rotator.rotate(key.transpose(-2, -1)).transpose(-2, -1)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(config.n_positions, config, scale=True, attn_index=index) for index in range(config.n_layer)])
