    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            if hasattr(m, "is_shortcut") and m.is_shortcut:
                nn.init.constant_(m.weight, thresh / (2**0.5))  # two brance
            else:
...
class SpikeDecisionTransformer(nn.Module):
    def __init__(self, config):
        ...
        super().__init__()
        T =  4  # SNN time step
        # the only difference between TSSA and PSSA is the attention type
...
class TSSADecisionSpikeFormer(TrajectoryModel):
    ...
    def __init__(
        ...
        print("Using DecisionSpikeFormer: TSSA")
