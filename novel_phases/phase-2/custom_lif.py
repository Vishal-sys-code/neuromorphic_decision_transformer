...
class CustomLIFCell(LIFCell):
    def __init__(
        ...
        # name: Optional[str] = None, # Removed: SNNCell does not expect 'name'
        **kwargs
    ):
        # super().__init__(p=p, dt=dt, name=name, **kwargs) # Removed name
        super().__init__(p=p, dt=dt, **kwargs) # Pass relevant args to parent
